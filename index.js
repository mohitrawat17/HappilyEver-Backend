...
app.post('/login', async (req, res) => {
  ...
  const user = await Warden.findOne({ universityID: req.body.universityID, password: req.body.password }, { _id: 0, __v: 0 })
  if (!user) {
...
app.get('/slots', async (req, res) => {
  ...
  try {
    ...
    const availableSlots = await Warden.find({ booked: false, universityID: { $ne: universityID } }, { _id: 0, password: 0, __v: 0, day: 0, sessionWith: 0 });
...
app.post('/book-slot', async (req, res) => {
  ...
  try {
    ...
    if (existingSlot) {
      ...
      user.booked = true;
      user.sessionWith.push({wardenName:existingSlot.name,day:day,time:time});
      await user.save();

      //updating user who is booked for meeting
      existingSlot.sessionWith.push({wardenName:user.name,day:day,time:time});
...
// // Route for fetching pending sessions for a warden
// app.get('/pending-sessions', async (req, res) => {
//   // const { wardenId } = req.query;

//   try {
//     const pendingSessions = await Slot.find({ wardenId: "102", booked: true });
